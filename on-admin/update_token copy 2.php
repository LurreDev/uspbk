...
// echo $hash; // Output: "b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9"

// //auto nama dan tanggal waktu jeda 2 menit
// $datetime = new DateTime();
// $datenow = $datetime->format('Y-m-d H:i:s');
// // $s=$nama.$datenow;

// //pengujian nama+tgl waktu
// $random_number = random_int(1, 100);
// $s = 'admin'.$random_number.' 2023-04-20 20:02:00';
// $hash = hash_sha256($s);
// $txt = $hash;
// $myfile = fopen("../on-siswa/token.txt", "w") or die("Unable to open file!");
// //ambil dari random
// $shorttoken =  substr($txt, 0, 6);
// //ambil dari tengah
// $shorttoken = substr($hash, 13, 6);
// //ambil dari belakang
// $shorttoken = substr($hash, 13, -6);

// fwrite($myfile, $shorttoken);
// fclose($myfile);
...
$waktu = $datenow;
$random_number = random_int(1, 100);
$kombinasikata =$random_number.$username.$datenow;
$encrpt = hash_sha256($kombinasikata);
//ambil dari awal
$shorttoken1 =  substr($encrpt, 0, 6);
//ambil dari tengah
$shorttoken2 = substr($encrpt, 13, 6);
//ambil dari belakang
$shorttoken3 = substr($encrpt, -6, 6);

// Cek apakah username dan waktu telah ada di database
$query1 = mysqli_query($konek, "SELECT * FROM token WHERE hasil_token ='$shorttoken1'");
 
if (mysqli_num_rows($query1)>0) {
    # code...
    // die("Data sudah ada");
    $query2 = mysqli_query($konek, "SELECT * FROM token WHERE hasil_token = '$shorttoken2'");
    if (mysqli_num_rows($query2)>0) {
            //   die("Data sudah ada");
        if ($add2 = mysqli_query($konek, "INSERT INTO token (username, waktu, encrpt, hasil_token) VALUES ('$username', '$waktu', '$encrpt', '$shorttoken3')")) {
            header('location:index.php?token=1');
            exit();
        } else {
        ...
        if ($add3 = mysqli_query($konek, "INSERT INTO token (username, waktu, encrpt, hasil_token) VALUES ('$username', '$waktu', '$encrpt', '$shorttoken2')")) {
                    header('location:index.php?token=1');
                    exit();
                } else {
                    die ("Terdapat kesalahan : ". mysqli_error($konek));
    ...
    # code...
    if ($add1 = mysqli_query($konek, "INSERT INTO token (username, waktu, encrpt, hasil_token) VALUES ('$username', '$waktu', '$encrpt', '$shorttoken1')")) {
        header('location:index.php?token=1');
        exit();
    } else {
        die ("Terdapat kesalahan : ". mysqli_error($konek));
